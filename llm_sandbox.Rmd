...
install.packages(c("ellmer", "httr", "jsonlite", "rvest", "usethis"))
```
...
chat$chat("Give me 20 names for a pet hamster")
...
basketballlll <- content_image_url("basketballlll.png")

extract_csv <- chat$chat("who is the best player in this image?")
...
```{r read-article}
article_text <- readLines("./data/ark_electric_hybrid.csv", warn = FALSE)
```
...
# Create prompt for structured data extraction
structured_response <- chat$chat(paste(
  "produce a structured list organizing this data with a the three most popular cities for  charging stations.Do not include any introductory text, no yapping.",
  "\nText:",
  sanctions_content
))
